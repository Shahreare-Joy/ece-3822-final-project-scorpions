...
class Game:
    ...
    def write_session_result(self, outcome="Quit"):
        ...
        if not result_path or self.level is None:
            return
        duration_seconds = int(getattr(self.level, "elapsed_seconds", lambda: max(0, (pygame.time.get_ticks() - self.level.start_time) // 1000))())
        result_label = getattr(self.level, "result", "lose")
        payload = {
            "player_id": self.player_name,
            "game_id": os.environ.get("SCORPIONS_GAME_ID", "crystal-run"),
            ...
            "metadata": {
                "score_type": "enemies_defeated",
                "selected_character": self.selected_character.get_display_name() if self.selected_character else "",
            },
        }
        try:
            ...
            print(f"[RESULT] Mystical Bamboo score submitted locally: {payload['score']}")
    ...
    def character_select(self):
        ...
        while char_select:
            ...
            if clicked_this_frame and selected_card and confirm_button_rect.collidepoint(mouse_pos):
                ...
                print(f"[GAME_4] Selected character: {self.selected_character.get_display_name()}")
                char_select = False
