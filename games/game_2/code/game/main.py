...
class Game:
    def __init__(self, player_name, server_host='localhost', server_port=8080, serializer='text'):
        # general setup
        pygame.init()
        self.screen = pygame.display.set_mode((WIDTH, HEIGTH))  # HEIGTH matches your settings.py typo
...
>>>>>>> main
    ...
    def run(self):
        ...
        Handles the play-again / return-to-arcade flow from the end screen:
          - 'restart'  -> rebuild the level with the same character and play again
          - 'arcade'   -> go back to the character-select screen
        """
...
>>>>>>> main
                ...
                # Check end-screen button result
                action = self.level._end_action
                if action == 'restart':
...
>>>>>>> main
            ...
            # If arcade was chosen, re-run character select then loop again
