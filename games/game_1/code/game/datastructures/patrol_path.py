"""
patrol_path.py - Linked list of waypoints for enemy patrol routes

Copy your completed implementation from Lab 5 into this file.
"""

# TODID: Copy your PatrolPath implementation from Lab 5 here
...
class PatrolPath:
    ...
    def __init__(self, patrol_type="circular"):
        ...
        """
        # TODID: Initialize head, tail, current to None
        # TODID: Store patrol_type
        # TODID: Initialize size to 0
        # TODID: Initialize direction to 1 (1 = forward, -1 = backward for back_and_forth)
    ...
    def add_waypoint(self, x, y, wait_time=0):
        ...
        """
        # TODID: Create a new Waypoint node
        # TODID: If the list is empty, set head, tail, and current to the new node
        # TODID: Otherwise, link the new node after the current tail
        # TODID: For "back_and_forth" and "circular", set prev pointer on the new node
        # TODID: Update tail to the new node
        # TODID: For "circular", close the loop (tail.next = head, head.prev = tail)
        # TODID: Increment size
    ...
    def get_next_waypoint(self):
        ...
        """
        # TODID: If empty or current is None, return None
        # TODID: Save current as result to return
        # TODID: For "one_way": advance current to current.next (becomes None at end)
        # TODID: For "circular": advance current to current.next (wraps around)
        # TODID: For "back_and_forth": advance forward or backward based on direction,
        ...
        if self.patrol_type == "one_way":
            ...
            if self.direction == 1:  # moving forward
                if self.current.next is not None:
                    ...
                    self.direction = -1  # reverse direction at end
                    self.current = self.current.prev
            else:  # moving backward
                if self.current.prev is not None:
                    self.current = self.current.prev
                else:
                    self.direction = 1  # reverse dir at start
                    self.current = self.current.next
